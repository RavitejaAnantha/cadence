...
def _recs(variant, seed=0, user="u1", situation="commute", k=K):
    catalog = build_catalog(seed=seed)
    users = build_users(seed=seed)
    return recommend(variant, get_user(users, user), Context(situation), catalog, k=k)


def check_determinism():
    a = [r.book.book_id for r in _recs("personalized")]
    b = [r.book.book_id for r in _recs("personalized")]
...
def check_no_dupes():
    ids = [r.book.book_id for r in _recs("personalized", k=10)]
...
def check_pop_not_in_truth():
    catalog = build_catalog(seed=0)
    users = build_users(seed=0)
    u, ctx, b = users[0], Context("commute"), catalog[0]
    r1 = true_relevance(u, ctx, b)
    r2 = true_relevance(u, ctx, replace(b, popularity=(b.popularity + 0.5) % 1.0))
...
def check_diverse_spreads_genres():
    catalog = build_catalog(seed=0)
    users = build_users(seed=0)
    u, ctx = get_user(users, "u1"), Context("commute")
    pers = {r.book.genre for r in recommend("personalized", u, ctx, catalog, k=5)}
    div = {r.book.genre for r in recommend("diverse", u, ctx, catalog, k=5)}
...
INTENTS = [
    ...
    ("recommend() never returns duplicate titles", check_no_dupes),
